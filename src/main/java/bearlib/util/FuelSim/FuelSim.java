...
public class FuelSim {
  ...
  /** Logging helper: returns the current fuel positions as an array for Epilogue logging. */
  @Logged(name = "Fuel Positions", importance = Importance.DEBUG)
  public edu.wpi.first.math.geometry.Translation3d[] getFuelPositionsForLog() {
    return fuels.stream().map(f -> f.pos).toArray(edu.wpi.first.math.geometry.Translation3d[]::new);
  }

  /** Logging helper: returns the current fuel count for Epilogue logging. */
  @Logged(name = "Fuel Count", importance = Importance.CRITICAL)
  public int getFuelCountForLog() {
    return fuels.size();
  }
