 ...
 *
 * <p>{ // options for logging private boolean useConsole = false; private boolean useDataLog =
 * true; private boolean useShuffleBoardLog = false;
 *
 * <p>configureCommandLogs(); // do early on otherwise log not ready for first
 * commands @SuppressWarnings("resource") public void configureCommandLogs() { if (useConsole ||
 * useDataLog || useShuffleBoardLog) { schedulerLog = new CommandSchedulerLog(useConsole,
 * useDataLog, useShuffleBoardLog); schedulerLog.logCommandInitialize();
 * schedulerLog.logCommandInterrupt(); schedulerLog.logCommandFinish();
 * schedulerLog.logCommandExecute(); // Can (optionally) generate a lot of output } else { new
 * Alert("No logging", AlertType.kWarning).set(true); } } }
 */
public class CommandSchedulerLog {
  private static final String m_fullClassName =
   ...
   *
   * <p>Set the command scheduler to log events for command initialize, interrupt, finish, and
   * execute.
   *
   ...
   *
   * <p>Note that use of the DataLog creates SmartDashboard/ShuffleBoard entries but are not the
   * same as use of the ShuffleBoardLog. The ShuffleBoardLog has event markers independent of the
   * DataLog.
   *
            ...
            (command) -> {
              ...
              if (m_useShuffleBoardLog) {
                Shuffleboard.addEventMarker(
                    "Command initialized", key + " " + requirements, EventImportance.kNormal);
              }
        ...
        .onCommandInterrupt(
            (command, interruptedBy) -> {
              ...
              if (interruptedBy.isPresent()) {
                interrupter =
                    "interrupted by command "
                        + command.getClass().getSimpleName()
                        + "/"
                        + interruptedBy.get().getName();
              } else {
                interrupter =
                    "interrupted"; // interrupted not by a command - mode change, cancelled,
                // timeOut, until, etc.
              }

              String key = command.getClass().getSimpleName() + "/" + command.getName();
              String runs =
                  " after " + m_currentCommands.getOrDefault(key, 0) + " runs " + interrupter;
              ...
              if (m_useShuffleBoardLog) {
                Shuffleboard.addEventMarker(
                    "Command interrupted", key + runs, EventImportance.kNormal);
              }
   ...
   *
   * <p>This can generate a lot of events so logging is suppressed except for the first occurrence
   * of execute(). Total count of execute() is logged at command end.
   *
